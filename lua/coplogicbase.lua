...
Hooks:PreHook(CopLogicBase, "on_new_objective", "sh_on_new_objective", function (data, old_objective)
	if not data.objective or data.objective.type ~= "defend_area" or not data.group or not data.tactics or (not data.tactics.shield_cover and not data.tactics.unit_cover) then
		return
	end
	...
	if not alive(shield_unit) or shield_unit:character_damage():dead() then
		...
		for u_key, u_data in pairs(data.group.units) do
			...
			if logic_data.tactics then
				...
				elseif not logic_data.tactics.unit_cover then
					unit_followers[u_key] = unit_followers[u_key] or 0
				end
			end
		end
		...
		if not best_shield_key then
			for u_key, followers in pairs(unit_followers) do
				if followers < least_followers then
					best_shield_key = u_key
					least_followers = followers
				end
