...
function CopLogicIdle._get_attention_weight(attention_data, att_unit, distance)
	...
	local peer_name = att_unit and att_unit:network() and att_unit:network():peer() and att_unit:network():peer():character() 
	local hostages_killed_att_mul = 1 + math.min(managers.trade:hostages_killed_by_name(peer_name) * tweak_data.player.hostage_killed_attention.add_mul, tweak_data.player.hostage_killed_attention.max_mul)
		
	if attention_data.is_local_player then
		local current_state = att_unit:movement():current_state()
			
		weight_mul = weight_mul * hostages_killed_att_mul
		
		if current_state:ducking() then
			weight_mul = weight_mul * managers.player:upgrade_value("player", "crouch_camouflage_bonus", 1)
		...
		if current_state and current_state:_interacting() and att_unit:anim_data() and att_unit:anim_data().revive then
			weight_mul = weight_mul * managers.player:upgrade_value("player", "revive_camouflage_bonus", 1)
		end
	...
	elseif attention_data.is_husk_player then
		...
		if movement._pose_code == 2 then
			weight_mul = weight_mul * managers.player:upgrade_value("player", "crouch_camouflage_bonus", 1)
			
			if not movement._move_data then
				weight_mul = weight_mul * managers.player:upgrade_value("player", "stand_still_crouch_camouflage_bonus", 1)				
			end
		end

		if att_unit:anim_data() and att_unit:anim_data().revive then
			weight_mul = weight_mul * base:upgrade_value("player", "revive_camouflage_bonus", 1)
		end
