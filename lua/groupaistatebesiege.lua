...
function GroupAIStateBesiege:_upd_assault_task(...)
	...
	if task_data.force_end or end_assault then
		...
		self._assaults_elapsed = self._assaults_elapsed + 1
		
		if self._assaults_elapsed >= (tweak_data.upgrades.assaults_survived_life_restore or 2) then	
			self._assaults_elapsed = 0
			
			for u_key, u_data in pairs(self._player_criminals) do
				if u_data.unit:base().is_local_player then
					if managers.player:has_category_upgrade("player", "survivor_life_restore") then
						u_data.unit:character_damage():restore_lives(1)
						PaperWarm:log("Assaults survived, restoring downs")
					end
				elseif u_data.unit:base():upgrade_value("player", "survivor_life_restore") then
					u_data.unit:character_damage():restore_lives(1)
					PaperWarm:log("Assaults survived, restoring downs")
				end
			end	
   ...
   if tactics_map.door_ambush then
	...
	elseif approach then
		...
		elseif not current_objective.assigned_t and in_place_duration > 15 and not self:_can_group_see_target(group) then
			...
			PaperWarm:warn("Group %s spawned from element %u (%s) is stuck, removing it!", group.id, element_id, element_name)
...
function GroupAIStateBesiege:_chk_group_use_grenade(assault_area, group, detonate_pos)
	...
	elseif use_frag then
		ProjectileBase.throw_projectile_npc("frag_npc", detonate_pos, Vector3(0, 0, 0), grenade_user.unit)

		timeout = tweak_data.group_ai.frag_grenade_timeout or tweak_data.group_ai.smoke_and_flash_grenade_timeout
	else
		...
		self:detonate_smoke_grenade(detonate_pos, mvec_cpy(grenade_user.m_pos), tweak_data.group_ai[grenade_type .. "_lifetime"] or 15, grenade_type == "flash_grenade", false)
...
--	self:_check_commander_difficulty_increase()
...
function GroupAIStateBesiege:_perform_group_spawning(spawn_task, force)
	...
	local function _try_spawn_unit(u_type_name, spawn_entry)
		...
		if not u_category then
			PaperWarm:error("Unit category %s does not exist", u_type_name)
			return true
		end
		...
		if hopeless then
			PaperWarm:warn("Spawn group %s failed to spawn unit %s", spawn_task.spawn_group.id, u_type_name)
			return true
		end
	end
...
function GroupAIStateBesiege:_choose_best_group(best_groups, total_weight)
	...
	for i, candidate in ipairs(best_groups) do
		...
		if rand_wgt <= 0 then
			local spawn_group_id = candidate.group.mission_element:id()
			self._spawn_group_timers[spawn_group_id] = TimerManager:game():time() + math.random(10, 15)
...
function GroupAIStateBesiege:_spawn_in_group(spawn_group, spawn_group_type, grp_objective, ai_task)
	...
	local function _add_unit_type_to_spawn_task(i, spawn_entry)
		...
		if wanted_nr_units < add_amount then
			...
			PaperWarm:warn("Can not satisfy amount_min for unit category %s in spawn group type %s", spawn_entry.unit, spawn_group_type)
		end
...
Hooks:OverrideFunction(GroupAIStateBesiege, "_set_reenforce_objective_to_group", function (self, group)
	...
	elseif next(target_area.criminal.units) then
		...
		elseif not self:_chk_group_use_grenade(target_area, group) then
			...
			if not group.in_place_t or self._t - group.in_place_t < no_grenade_push_delay * 0.5 then
				return
			end
		end
	end
...
function GroupAIStateBesiege:commander_assault_enabled()
...
function GroupAIStateBesiege:commander_assault_disabled()
	self:set_assault_endless(false)	
	
	if self._commander_diff_i then
		self:add_difficulty(-(self._commander_diff_i * tweak_data.groupai.commander.difficulty.increase))
	end
...
function GroupAIStateBesiege:commander_difficulty_increase_enable()
	self._commander_difficulty_last_increase = self._commander_difficulty_last_increase or TimerManager:game():time()
end


function GroupAIStateBesiege:commander_difficulty_increase_disable()
	self._commander_difficulty_last_increase = nil
end


function GroupAIStateBesiege:_check_commander_difficulty_increase()
	if self._difficulty_value < 1 and self._commander_difficulty_last_increase then
		local t = TimerManager:game():time()
		local increase_interval =  tweak_data.group_ai.commander and tweak_data.group_ai.commander.difficulty.interval
		local last_increase = self._commander_difficulty_last_increase
		local commander_unit = self._commander_unit
		
		if t > last_increase + math.lerp(increase_interval[1], increase_interval[2], math.random()) then
			last_increase = t
			...
			commander_unit:movement():action_request(action_data)
	
			self:add_difficulty(tweak_data.groupai.commander.difficulty.increase)
			self._commander_diff_i = (self._commander_diff_i or 0) + 1
			
			managers.hud:show_hint( { text = managers.localization:text("hud_assault_commander_diff_increase") } )

			self._commander_difficulty_last_increase = last_increase
		end
	end
end
]]