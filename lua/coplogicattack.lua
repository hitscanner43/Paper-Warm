...
function CopLogicAttack._chk_wants_to_take_cover(data, my_data)
	...
	if data.attention_obj.verified and data.attention_obj.dis < my_data.weapon_range.close then
		return true
	end
...
function MedicLogicAttack._upd_aim(data, my_data)
	local focus_enemy = data.attention_obj
	local verified = focus_enemy and focus_enemy.verified
	local nearly_visible = focus_enemy and focus_enemy.nearly_visible
...
function MedicLogicAttack._check_aim_shoot(data, my_data, focus_enemy, verified, nearly_visible)
	if not focus_enemy or focus_enemy.reaction < AIAttentionObject.REACT_AIM then
		return
	end
	...
	local expected_pos
	local aim = data.char_tweak.always_face_enemy or not advancing or pushing or verified_dt < math.map_range(enemy_dis, 0, firing_range, 6, 3)
	local shoot = aim and my_data.shooting and AIAttentionObject.REACT_SHOOT <= focus_enemy.reaction and verified_dt < 2 
	...
	if verified or nearly_visible then
		if not shoot and AIAttentionObject.REACT_SHOOT <= focus_enemy.reaction then
			...
			if suppression_dt and suppression_dt < 3.5 * (verified and 1 or 0.5) then
				shoot = true
			elseif verified and enemy_dis < firing_range then
				shoot = true
			elseif verified and assault_dt and assault_dt < 2 then
				shoot = true
			elseif my_data.attitude == "engage" and my_data.firing and verified_dt < 4 then
				shoot = true
			end
...
--Grenadier attack logic
GrenadierLogicAttack = GrenadierLogicAttack or class(CopLogicAttack)
function GrenadierLogicAttack._upd_aim(data, my_data, ...)
	local focus_enemy = data.attention_obj
	local visible = focus_enemy and (focus_enemy.verified or focus_enemy.nearly_visible)
	local aim, shoot, expected_pos
	if focus_enemy then
		if GrenadierLogicAttack._chk_use_throwable(data, my_data, focus_enemy) then
			return
		end
...
function GrenadierLogicAttack._chk_use_throwable(data, my_data, focus)
	...
	if data.char_tweak.throwable_lethal and not has_hostages or data.char_tweak.collateral_damage then
		throwable = data.char_tweak.throwable_lethal
	end
	
	local throwable_tweak = tweak_data.projectiles[throwable]
	local weapon_usage_tweak = data.char_tweak.weapon[data.unit:inventory():equipped_unit():base():weapon_tweak_data().usage]
	...
	local is_throwable = tweak_data.blackmarket.projectiles[throwable].throwable or tweak_data.blackmarket.projectiles[throwable].throwable_npc
	...
	local min_throw_dis = is_throwable and 400 or weapon_usage_tweak.range.close
	local max_throw_dis = is_throwable and 1600 or weapon_usage_tweak.range.far
	
	if min_throw_dis > throw_dis or max_throw_dis < throw_dis then
		return
	end
	...
	local lerp_dis = math.min(1, throw_dis / max_throw_dis)
	local aim_delay = weapon_usage_tweak.aim_delay_throw or weapon_usage_tweak.aim_delay 
	local throw_delay = math.lerp(aim_delay[1], aim_delay[2], lerp_dis)
	
	if not data.delay_throw then
		data.throw_delay_t = data.t + throw_delay
	end

	data.delay_throw = true
		
	if data.throw_delay_t and data.t > data.throw_delay_t then
...
CommanderLogicAttack = CommanderLogicAttack or class(CopLogicAttack)

function CommanderLogicAttack.enter(data)
	managers.groupai:state():commander_difficulty_increase_enable()
	managers.groupai:state():commander_assault_enabled()	
	managers.groupai:state():register_commander(data.unit)
end


function CommanderLogicAttack.exit()
	managers.groupai:state():commander_difficulty_increase_disable()
	managers.groupai:state():commander_assault_disabled()
	managers.groupai:state():unregister_commander()
end